...
handle('env:status', async () => {
  const env = await gpg.status();
  const mine = env.keys.find((k) => k.onCard) || null;
  return { ...env, myKey: mine, appVersion: APP_VERSION };
});
 ...
 */
handle('encrypt:key', async ({ filePath, recipientId, signerId }) => {
  const out = encryptedPath(filePath);
  if (signerId) await gpg.signEncryptToKey(filePath, out, recipientId, signerId);
  else await gpg.encryptToKey(filePath, out, recipientId);
